...
/**
 * Main usage and entry point of Library. Provides a mapping facade based on mapping metadata collected earlier from {@link
 * com.naharoo.commons.mapstruct.BaseMapper} extensions/children.
 * <p>
 * This interface is Spring-friendly. It can be easily injected into any business bean. Avoid injecting and using this Bean before Spring
 * ApplicationContext's complete initialization.
 * <p>
 * Uses {@link com.naharoo.commons.mapstruct.MappingsRegistrationBeanPostProcessor} underneath to collect mapping metadata in Spring
 * ApplicationContext's initialization phase.
 *
...
public interface MappingFacade {
    ...
    /**
     * Maps <code>source</code> object into <code>destinationClass</code> type. Afterwards, applies the provided customization on the destination.
     *
     * @param source                Original source Object which will be mapped into
     *                              <code>destinationClass</code>
     * @param destinationClass      <code>destinationClass</code> which is used as a metadata to resolve
     ...
     * @throws MappingNotFoundException if there is no <code>S</code> -> <code>D</code> mapping registered
     */
    @PublicApi <S, D> D map(@Nullable S source, @NonNull Class<D> destinationClass, @NonNull Consumer<D> destinationCustomizer);
     ...
     * @throws MappingNotFoundException if there is no <code>S</code> -> <code>D</code> mapping registered
     */
    @PublicApi <S, D> List<D> mapAsList(@Nullable Collection<S> sources, @NonNull Class<D> destinationClass);

    /**
     * Maps <code>sources</code> collection into a list of objects having <code>destinationClass</code> type. Afterwards, applies the provided
     * customization on destinations.
     *
     * @param sources               Original list of source Objects which will be mapped into a list of objects having
     *                              <code>destinationClass</code> type
     * @param destinationClass      <code>destinationClass</code> which is used as a metadata to resolve
     ...
     * @throws MappingNotFoundException if there is no <code>S</code> -> <code>D</code> mapping registered
     */
    @PublicApi <S, D> List<D> mapAsList(
     ...
     * @throws MappingNotFoundException if there is no <code>S</code> -> <code>D</code> mapping registered
     */
    @PublicApi <S, D> Set<D> mapAsSet(@Nullable Collection<S> sources, @NonNull Class<D> destinationClass);

    /**
     * Maps <code>sources</code> collection into a set of objects having <code>destinationClass</code> type. Afterwards, applies the provided
     * customization on destinations.
     *
     * @param sources               Original set of source Objects which will be mapped into a set of objects having
     *                              <code>destinationClass</code> type
     ...
     * @throws MappingNotFoundException if there is no <code>S</code> -> <code>D</code> mapping registered
     */
    @PublicApi <S, D> D[] mapAsArray(@Nullable Collection<S> sources, @NonNull Class<D> destinationClass);
     ...
     * @throws MappingNotFoundException if there is no <code>S</code> -> <code>D</code> mapping registered
     */
    @PublicApi <S, D> D[] mapAsArray(@Nullable S[] sources, @NonNull Class<D> destinationClass);
}
